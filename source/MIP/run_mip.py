...
def build_model(n: int, approach: str, time_limit: int = 300, seed: int = 42):
    ...
    # Model creation
    model_name = f"STS_MILP_{approach}_n{n}"
    prob = pulp.LpProblem(model_name, pulp.LpMinimize)

    # Objective
    if approach.lower() == "balance":
    ...
    if pulp_status in ("Optimal", "Integer Feasible", "Not Solved", "Infeasible","Undefined") and pulp_status != "Infeasible" and pulp_status != "Undefined":
        if pulp_status == "Not Solved":
            ...
            if not has_values:
                sol_matrix = ["Solution not found"] # METTI TIME 300 SOL VUOTA
        ...
        sol_matrix = ["Infeasible or Undefined"]

    # Compute objective value (or None)
    obj_val = None
    if approach.lower() == "balance" and solved_to_optimality:
        obj_val = int(round(pulp.value(prob.objective)))
    elif approach.lower() != "balance":
        obj_val = None
...
def main():
    ...
    result, meta = build_model(n, "feasibility", time_limit=300, seed=26)  # "feasibility" , "balance"
