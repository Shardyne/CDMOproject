...
def build_model(n: int, time_limit: int = 300, seed: int = 0):
    ...
    # --- Use unordered-pair variables y[(w,p,i,j)] only for i < j (removes i/j duplication) ---
    x = {}
    for w in W:
        for p in P:
            for i in T:
                for j in T:
                    if i < j:                                 # only one variable per unordered pair
    ...
    d = pulp.LpVariable.dicts("imbalance", T, lowBound=0, cat=pulp.LpInteger) #cat=pulp.LpContinuous)
    # Define h_i - a_i
    home_minus_away = {i:
    ...
    # (1) Each (week, period) hosts exactly one match (orientation irrelevant at this stage)
    for w in W:
        for p in P:
            prob += pulp.lpSum(x[(w,p,i,j)] for i in T for j in T if i < j) == 1, f"one_match_w{w}_p{p}"

    # (2) Each unordered pair (i<j) occurs exactly once across all weeks/periods
    for i in T:
        for j in T:
            if i < j:
                prob += pulp.lpSum(x[(w,p,i,j)] for w in W for p in P) == 1, f"pair_{i}_{j}"

    # --- Precompute unordered pairs and index by team (no min/max inside lpSum) ---
    pairs = [(i, j) for i in T for j in T if i < j]               # tutte le coppie i<j
    pairs_of_team = {t: [] for t in T}                             # map team -> [(i,j), ...]
    for (i, j) in pairs:
    ...
    for t in T:
        for w in W:
            # sum over all unordered pairs in week w that contain team t
    ...
    # ---------------------------
    # Symmetry breaking: anchor team k in week1 period1 (A)
    # ---------------------------
    # if n > 4:
    #     k = 1  # modifica k manualmente se vuoi testare un altro anchor
    #     # sum over unordered pairs containing k in slot (1,1)
    #     prob += pulp.lpSum(x[(1, 1, i, j)] for (i, j) in pairs_of_team[k]) == 1, f"sym_team_{k}_w1_p1"

    # ---------------------------
    # (D) Fix composition of first week via circle method (sparse, safe)
    # ---------------------------
    # pairs_week1 = compute_circle_pairs(n)
    # for (a, b) in pairs_week1:
    #     ia, ib = (a, b) if a < b else (b, a)   # compute once (small cost, but clearer)
...
if __name__ == '__main__':
    for x in [0, 1234567, 1756566010, 26, 42]:
        n = 12
        res_dir = os.path.join(os.path.dirname(__file__), "..", "..", "res", "MIP", "v3_seed_balanced")
        os.makedirs(res_dir, exist_ok=True)
        out_path = os.path.join(res_dir, f"v3_{x}_{n}.json")
