...
def build_model(n: int, approach: str, time_limit: int = 300, seed: int = 42):
    ...
    if approach.lower() == "balance":
        ...
        d = pulp.LpVariable.dicts("imbalance", T, lowBound=0, cat=pulp.LpInteger)
        # Define h_i - a_i
        home_minus_away = {i:
    ...
    # Solve
    solver = pulp.PULP_CBC_CMD(msg=True, timeLimit=time_limit, threads=1, options=["-randomSeed", str(seed)])
    start = time.time()
    ...
    sol_matrix = None
    if pulp_status in ("Optimal", "Integer Feasible", "Infeasible") and pulp_status != "Infeasible":
