...
#         # linearize absolute value: -d <= home_minus_away <= d
#         for i in T:
...
#     # Solve
#     # ---------------------------
#     solver = pulp.PULP_CBC_CMD(msg=True, timeLimit=time_limit, presolve=presolve, threads=1, options=[f"RandomS {seed}"])
#     start = time.time()
...
#     for version in ["base","i!=j"]:# "base",
#         for objective in ["balanced", "feasible"]:#"feasible",
#             for seed in [262626, 948486489, 5656565, 878641, 1234567,0]:#0, 1234567, 1756566010, 26, 42
#                 # for presolve in [False]:#  ----> meglio true si stabilizzano meglio i balanced
#                 for sym_flags in ["AC","BC","B","D"]:
#                     for n in range(12,17,2):
#                         res_dir = os.path.join(os.path.dirname(__file__), "..", "..", "res", "MIP", "bin_1",f"{version}_{objective}") #, "bin"
...
#                                                         presolve=True, version=version,
#                                                         objective=objective, sym_flags= sym_flags)
...
#                         key = f"CBC_{version}_{objective}"
#                         payload = { key: result }
...
# def build_model(n: int, time_limit: int = 300, seed: int = 0,
#                 presolve: bool = False, objective: str = "feasible",
#                 version: str = "base", sym_flags: str = "", warmstart: str = ""):
#     """
#     build_model con opzione sym_flags: stringa che può contenere le lettere
#     'A','B','C','D' per abilitare rispettivamente i symmetry-breaking:
#       A = anchor team k in week1 period1
...
#     warmstart: stringa che può essere
#       ""       -> nessun warm start
#       "circle" -> warm start completo con circle_method_pairs(n)
#       "week1"  -> warm start solo prima settimana con compute_circle_pairs(n)
#     """
...
#     pairs = [(i, j) for i in T for j in T if i < j]
#     pairs_of_team = {t: [] for t in T}
#     for (i, j) in pairs:
#         pairs_of_team[i].append((i, j))
#         pairs_of_team[j].append((i, j))
...
#     # --- Warm start ---
#     if warmstart == "circle":
#         init_sched = circle_method_pairs(n)
#         for w, pairs in init_sched.items():
#             for p, (i, j) in enumerate(pairs, start=1):
#                 if version == "i<j" and i > j:
#                     i, j = j, i
#                 if (w,p,i,j) in x:
#                     x[(w,p,i,j)].setInitialValue(1)
#                     x[(w,p,i,j)].fixValue()
#     elif warmstart == "week1":
#         pairs = compute_circle_pairs(n)
#         w = 1
#         for p, (i, j) in enumerate(pairs, start=1):
#             if version == "i<j" and i > j:
#                 i, j = j, i
...
#     return prob, status_code, runtime
# ---------------------------------------------------------------

    # # ---------- WARM START HANDLING ----------
    # # warm_start parameter accepted values:
    # #   "" -> no warm start
    # #   "full" -> full circle method warm-start (all weeks)
    # #   "week1" -> warm-start only week 1 (compute_circle_pairs)
    # ws = (warm_start).lower()
    # do_warm_start = ws in ("full", "week1")
    ...
    #     # Complete warm start
    #     if ws == "full":
    #         schedule = circle_method_pairs(n)  # dict: week -> list of (teamA,teamB)
    #         for wk, pairs in schedule.items():
    #             for per_idx, (a, b) in enumerate(pairs, start=1):
    ...
    #     # One week warm start
    #     elif ws == "week1":
    #         pairs = compute_circle_pairs(n)  # list of (i,j) for periods 1..n/2
    #         wk = 1
    #         for per_idx, (a, b) in enumerate(pairs, start=1):
    #             _set_x_initial(wk, per_idx, a, b)
...
# for a, b in enumerate(range(1,8)):
#     if a % 2 == 0:
#         continue
#     print(a,b)