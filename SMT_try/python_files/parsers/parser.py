#!/usr/bin/env python3
# parse_smt_and_update_json_bv.py
import argparse, json, os, re, subprocess, sys, time
from collections import defaultdict

# ---------- helpers to parse solver stdout ----------
DEF_FUN_ANY_RE = re.compile(
    r"\(define-fun\s+([A-Za-z0-9_]+)\s+\(\)\s+([A-Za-z0-9_]+)\s+([^\s\)]+)\)", re.I
)
GET_VAL_PAIR_RE = re.compile(r"\(\s*([A-Za-z0-9_]+)\s+(true|false|-?\d+)\s*\)", re.I)

def _coerce_lit(sort: str, lit):
    """
    Convert SMT-LIB literal to Python int/bool.
    Works for QF_LIA and QF_BV (BitVec outputs).
    """
    s = sort.lower()
    try:
        val = getattr(lit, "as_long", lambda: int(lit))()
    except Exception:
        val = lit
    if s == "bool":
        return bool(val)
...
def parse_model(stdout: str) -> dict:
    ...
    Supports define-fun and get-value styles.
    Works for int, bool, and BV integer results.
    """
    vals = {}
...
# ---------- main ----------
def main():
    ap = argparse.ArgumentParser(description="Parse SMT2 model and update JSON (supports QF_BV)")
    ...
    outpath = os.path.join(args.outdir, f"{args.N or 'unknownN'}.json")

    if stdout == "timeout":
        payload = {approach: {"time": args.timeout, "optimal": False, "obj": None, "sol": []}}
        existing = {}
    ...
    if status_line.startswith("unsat") or status_line.startswith("unknown"):
        payload = {approach: {"time": int(elapsed), "optimal": False, "obj": None, "sol": []}}
        existing = {}
        if os.path.exists(outpath):
            try: existing = json.load(open(outpath))
            except Exception: pass
    ...
    if not assigns:
        ...
        raise SystemExit("Could not parse any variable assignments.")
    ...
    W = W1 or W2 or W3
    if not T or not W:
        raise SystemExit("Could not infer N/W. Pass --N explicitly.")
    P = T // 2

    # Build grids
    Per = read_grid(assigns, "Per", T, W) if T1 else None
    Home = read_grid(assigns, "Home", T, W, default=False) if T2 else [[False]*W for _ in range(T)]
    Opp = read_grid(assigns, "Opp", T, W) if T3 else None
    ...
    existing[approach] = {"time": int(elapsed), "optimal": True, "obj": obj, "sol": sol}
    with open(outpath, "w") as f: json.dump(existing, f)
